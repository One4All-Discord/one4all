...
module.exports = class roleDelete extends Event {
	...
	async run(client, role) {
		...
		if (!isOn) return;
		let action = await guild.fetchAuditLogs({type: AuditLogEvent.RoleDelete}).then(async (audit) => audit.entries.first());
		...
		if (isGuildOwner || isBotOwner || isWlBypass && isWl) return console.log(`No sanction  ${isWlBypass && isWl ? `whitelisted` : `guild owner list or bot owner`} - ROLE_DELETE`);
			...
			if (member.roles.highest.comparePositionTo(role.guild.members.me.roles.highest) <= 0) {
				...
				if (sanction === 'ban') {
					...
					if (action.executor.bot) {
						...
						await botRole.setPermissions(0n, `OneForAll - Type: roleDelete`)
					}
				}
				if(channel){
