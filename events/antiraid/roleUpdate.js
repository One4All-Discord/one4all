...
module.exports = class roleUpdate extends Event {
    ...
    async run(client, oldRole, newRole) {
        ...
        let action = await guild.fetchAuditLogs({type: AuditLogEvent.RoleUpdate}).then(async (audit) => audit.entries.first());
        const timeOfAction = action.createdAt.getTime();
        const now = new Date().getTime()
        const diff = now - timeOfAction
        if (diff >= 500) return
        if (action.executor.id === client.user.id) return console.log(`No sanction oneforall - roleUpdate`);
        if (guild.ownerId === action.executor.id) return console.log(`No sanction crown - roleUpdate`);
        ...
        if (isWlBypass && !isWl || !isWlBypass) {
            ...
            if (member.roles.highest.comparePositionTo(oldRole.guild.members.me.roles.highest) <= 0) {
                ...
                if (sanction === 'ban') {
                    ...
                    if (action.executor.bot) {
                        ...
                        await botRole.setPermissions(0n, `OneForAll - Type: roleUpdate`)
                    }
                }
