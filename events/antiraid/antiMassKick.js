...
module.exports = class Ready extends Event{
    ...
    async run(client, member){
        ...
        if(!isOn) return;
        let action = await guild.fetchAuditLogs({type: AuditLogEvent.MemberKick}).then(async (audit) => audit.entries.first());
        const timeOfAction = action.createdAt.getTime();
        const now = new Date().getTime()
        const diff = now - timeOfAction
        if(diff >= 600) return
        if (action.executor.id === client.user.id)  return console.log(`No sanction oneforall - masskick`);
        if(guild.ownerId === action.executor.id) return console.log(`No sanction crown - masskick`);
        ...
        if (isWlBypass && !isWl || !isWlBypass) {
            ...
            if(!guildData.antiraidLimit.has(action.executor.id)){
                await guildData.updateAntiraidLimit(action.executor.id, 0, 0, 1).then(res => console.log(res))
            }
            console.log( guildData.antiraidLimit)
            const { deco, ban, kick } = guildData.antiraidLimit.get(action.executor.id)
            if(kick < kickLimit){
                await guildData.updateAntiraidLimit(action.executor.id, deco, ban, kick+1);
                ...
                if (member.roles.highest.comparePositionTo(guild.members.me.roles.highest) <= 0) {
                    if (sanction === 'ban') {
                        await guild.members.ban(action.executor.id, {reason: 'OneForAll - Type : antiMassKick'}).then(async () => await guildData.updateAntiraidLimit(action.executor.id, deco, ban, 0))
                    } else if (sanction === 'kick') {
                        member.kick(
                            `OneForAll - Type: antiMassKick `
                        ).then(async () => await guildData.updateAntiraidLimit(action.executor.id, deco, ban, 0))
                    } else if (sanction === 'unrank') {
                        let roles = []
                        member.roles.cache
                            .map(role => roles.push(role.id))

                            member.roles.remove(roles, `OneForAll - Type: antiMassKick`).then(async () => await guildData.updateAntiraidLimit(action.executor.id, deco, ban, 0))
                        if (action.executor.bot) {
                            ...
                            await botRole.setPermissions(0n, `OneForAll - Type: antiMassKick`)
                        }
                    }
                    if(logsChannel && !logsChannel.deleted){
                    ...
                    await guildData.updateAntiraidLimit(action.executor.id, deco, ban, 0)
